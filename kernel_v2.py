...
def dequantize_kernel_v2(
    ...
    weights_second = tl.load(weights_code_ptr + (weights_quant & 0x0F)) * absmax

    # Interleave the two going from [0, 2, 4], [1, 3, 5] -> [0, 1, 2, 3, 4, 5].
    weights = tl.reshape(
        tl.interleave(weights_first, weights_second), 2 * BLOCK_SIZE, can_reorder=False
    )

    output_offset = tl.program_id(0) * 2 * BLOCK_SIZE + tl.arange(0, 2 * BLOCK_SIZE)
    output_mask = output_offset < 2 * numel_weights
    tl.store(weights_ptr + output_offset, weights, mask=output_mask)


def dequantize_triton_v2(quant_weights, quant_state):
    ...
    dequantize_kernel_v2[launch_grid](
        weights_ptr=weights,
