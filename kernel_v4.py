...
@triton.jit
def dequantize_weights_v4(
    weights_ptr, weights_quant_ptr, weights_code_ptr,
    absmax_quant_ptr, absmax_code_ptr, absmax_scale_ptr, absmax_mean,
    numel_weights, blocksize_weights, blocksize_absmax,
    BLOCK_SIZE: tl.constexpr, BLOCK_STRIDE: tl.constexpr,
):
    numel_absmax = numel_weights // blocksize_weights
    base = tl.program_id(0) * 2 * BLOCK_SIZE

    for offset in tl.range(0, BLOCK_STRIDE, BLOCK_SIZE, num_stages=1):
        curr = base + offset
        logical_offset = curr + tl.arange(0, 2 * BLOCK_SIZE)
        ...
        quantized_absmax = tl.load(absmax_quant_ptr + absmax_offset, mask=absmax_mask)
        quantized_absmax = quantized_absmax.to(tl.int16)
        ...
        absmax_quant = tl.load(absmax_code_ptr + quantized_absmax, mask=absmax_mask)
        absmax = absmax_quant * absmax_scale + absmax_mean
        ...
        tl.store(
            weights_ptr + logical_offset, weights.to(tl.float16), mask=logical_mask
        )


def dequantize_triton_v4(quant_weights, quant_state):
    ...
    dequantize_weights_v4[launch_grid](
        weights_ptr=weights,
        ...
        BLOCK_STRIDE=max(256, min(16, quant_weights.numel() // 512)),
    )
